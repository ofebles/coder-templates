...
data "coder_parameter" "python_version" {
  name        = "Python Version"
  type        = "select"
  description = "Which Python version would you like?"
  mutable     = false
  default     = "3.12"
...
data "coder_parameter" "framework" {
  name        = "Framework"
  type        = "select"
  description = "Which Python web framework would you like?"
  mutable     = false
  default     = "fastapi"

  option {
    name  = "FastAPI (Modern, Fast)"
    value = "fastapi"
  }

  option {
    name  = "Flask (Lightweight, Flexible)"
    value = "flask"
  }

  option {
    name  = "Django (Full-featured, Batteries Included)"
    value = "django"
  }

  option {
    name  = "None (Plain Python)"
    value = "none"
  }
}
...
# JetBrains module - includes PyCharm and other IDEs
module "jetbrains" {
  count      = data.coder_workspace.me.start_count
...
resource "docker_image" "main" {
  name = "coder-python-${data.coder_workspace_owner.me.id}:latest"
  build {
    context    = path.module

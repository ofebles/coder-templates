...
data "coder_parameter" "go_version" {
  name        = "Go Version"
  type        = "select"
  description = "Which Go version would you like?"
  mutable     = false
  default     = "1.21"
...
data "coder_parameter" "framework" {
  name        = "Framework"
  type        = "select"
  description = "Which Go web framework would you like?"
  mutable     = false
  default     = "gin"

  option {
    name  = "Gin (Fast, Popular)"
    value = "gin"
  }

  option {
    name  = "Echo (Lightweight, Flexible)"
    value = "echo"
  }
...
# JetBrains module - includes GoLand and other IDEs
module "jetbrains" {
  count      = data.coder_workspace.me.start_count
...
resource "docker_image" "main" {
  name = "coder-go-${data.coder_workspace_owner.me.id}:latest"
  build {
    context    = path.module
